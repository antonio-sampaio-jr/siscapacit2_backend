...
router.get("/listarServidor/:id", async (request, response) => {
  try {
    ...
    const getGovEmployee = await GovEmployeeModel.findById(id)
      .populate("inscricoesAbertas")
      .populate("emAndamento")
      .populate("concluido");
...
// Essa rota me parece desnecessária, porque todas as informações daqui já estão em "3. Listar Servidor by ID (LIST)"
// mas, se a decisão for por incluí-la, ela precisa ser subdividida em 3: "Inscrições Abertas", "Em Andamento" e "Concluído"
